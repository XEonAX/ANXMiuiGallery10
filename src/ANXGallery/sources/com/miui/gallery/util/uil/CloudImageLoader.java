...
public class CloudImageLoader implements Callback {
    ...
    private abstract class ViewAware implements Comparable<ViewAware> {
        ...
        public boolean equals(Object o) {
            return o != null && (o instanceof ViewAware) && getImageView() == ((ViewAware) o).getImageView();
        }
    ...
    private class NonViewAwareImp extends ViewAware {
        private CloudImageLoadingListener mLoadingListener;
        private CloudImageLoadingProgressListener mProgressListener;

        public NonViewAwareImp(Uri uri, DownloadType imageType, CloudImageLoadingListener loadingListener, CloudImageLoadingProgressListener progressListener) {
            super(uri, imageType);
    ...
    private class WeakRefViewAwareImp extends ViewAware {
        private WeakReference<ImageView> mViewRef;

        public WeakRefViewAwareImp(Uri uri, DownloadType imageType, ImageView view, DisplayImageOptions displayImageOptions, ImageSize imageSize, CloudImageLoadingListener listener, CloudImageLoadingProgressListener progressListener, RectF regionRect, boolean needDisplay) {
            super(uri, imageType);
            CloudImageHolder.getImageHolder(view).setUri(uri).setImageType(imageType).setDisplayImageOptions(displayImageOptions).setImageSize(imageSize).setImageLoadingListener(listener).setImageLoadingProgressListener(progressListener).setRegionDecodeRect(regionRect).setNeedDisplay(needDisplay);
            this.mViewRef = new WeakReference(view);
        }

        public WeakRefViewAwareImp(CloudImageLoader cloudImageLoader, Uri uri, DownloadType imageType, ImageView view, DisplayImageOptions displayImageOptions, ImageSize imageSize, CloudImageLoadingListener listener, CloudImageLoadingProgressListener progressListener, RectF regionRect) {
            this(uri, imageType, view, displayImageOptions, imageSize, listener, progressListener, regionRect, true);
        }
        ...
        public Uri getUri() {
            ...
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getUri();
            }
            return null;
        }
        ...
        public DownloadType getDownloadType() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getImageType();
            }
            return null;
        }
        ...
        public CloudImageLoadingListener getLoadingListener() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getImageLoadingListener();
            }
            return null;
        }
        ...
        public CloudImageLoadingProgressListener getLoadingProgressListener() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getImageLoadingProgressListener();
            }
            return null;
        }
        ...
        public DisplayImageOptions getDisplayOptions() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getDisplayImageOptions();
            }
            return null;
        }
        ...
        public ImageSize getImageSize() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getImageSize();
            }
            return null;
        }
        ...
        public RectF getRegionDecodeRect() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).getRegionDecodeRect();
            }
            return super.getRegionDecodeRect();
        }

        public boolean needDisplay() {
            View view = getImageView();
            if (view != null) {
                return CloudImageHolder.getImageHolder(view).needDisplay();
            }
            return true;
        }
    }
        ...
        dispatchListener(wrapper, new Caller() {
            public void call(ViewAware aware) {
                ...
                if (loadingListener != null) {
                    loadingListener.onLoadingStarted(wrapper.uri, wrapper.type, aware.getImageView());
                }
            }
        }, false);
        return true;
    ...
    private boolean onSuccessLoading(final DownloadWrapper wrapper) {
        ...
        dispatchListener(wrapper, new Caller() {
            public void call(ViewAware aware) {
                CloudImageLoadingListener loadingListener = aware.getLoadingListener();
                if (loadingListener != null) {
                    loadingListener.onDownloadComplete(wrapper.uri, wrapper.type, aware.getImageView(), wrapper.downloadedItem.getFilePath());
                }
                if (aware.needDisplay()) {
                    ImageView view = aware.getImageView();
                    if (view != null) {
                        ...
                        ImageLoader.getInstance().displayImage(Scheme.FILE.wrap(wrapper.downloadedItem.getFilePath()), new ImageViewAware(view, false), options, aware.getImageSize(), new DecodeListenerWrapper(aware), null, aware.getRegionDecodeRect());
                    }
                } else if (loadingListener != null) {
                    loadingListener.onLoadingComplete(wrapper.uri, wrapper.type, aware.getImageView(), null);
                }
            }
        }, true);
        return true;
    ...
    private boolean onCancelLoading(final DownloadWrapper wrapper) {
        ...
        dispatchListener(wrapper, new Caller() {
            public void call(ViewAware aware) {
                CloudImageLoadingListener loadingListener = aware.getLoadingListener();
                if (loadingListener != null) {
                    loadingListener.onLoadingCancelled(wrapper.uri, wrapper.type, aware.getImageView());
                }
            }
        }, true);
        return true;
    ...
    private boolean onFailLoading(final DownloadWrapper wrapper) {
        ...
        dispatchListener(wrapper, new Caller() {
            public void call(ViewAware aware) {
                CloudImageLoadingListener loadingListener = aware.getLoadingListener();
                if (loadingListener != null) {
                    loadingListener.onLoadingFailed(wrapper.uri, wrapper.type, aware.getImageView(), wrapper.failReason == null ? ErrorCode.UNKNOWN : wrapper.failReason.getCode());
                }
            }
        }, true);
        return true;
        ...
        dispatchListener(wrapper, new Caller() {
            public void call(ViewAware aware) {
                ...
                if (progressListener != null) {
                    progressListener.onProgressUpdate(wrapper.uri, wrapper.type, aware.getImageView(), (int) wrapper.curSize, (int) wrapper.totalSize);
                }
            }
        }, false);
        return true;
    ...
    public void displayImage(Uri uri, DownloadType imageType, ImageView imageView, DisplayImageOptions displayImageOptions, ImageSize imageSize, CloudImageLoadingListener listener, CloudImageLoadingProgressListener progressListener, RectF regionDecodeRect, boolean delayRequest, boolean showLoadingImage) {
        if (uri == null) {
            ...
            if (displayImageOptions != null && displayImageOptions.shouldShowImageForEmptyUri()) {
                directShowImage(imageView, displayImageOptions.getImageForEmptyUri(imageView.getResources()));
            }
            onInvalidDownloadItem(uri, imageType, imageView, listener);
            return;
        }
        ...
        if (id <= 0) {
            ...
            if (displayImageOptions != null && displayImageOptions.shouldShowImageOnFail()) {
                directShowImage(imageView, displayImageOptions.getImageOnFail(imageView.getResources()));
            }
            if (listener != null) {
                listener.onLoadingFailed(uri, imageType, imageView, ErrorCode.UNKNOWN);
                return;
            }
            return;
        }
        if (displayImageOptions != null && displayImageOptions.shouldShowImageOnLoading() && showLoadingImage) {
            directShowImage(imageView, displayImageOptions.getImageOnLoading(imageView.getResources()));
        }
        ViewAware aware = new WeakRefViewAwareImp(this, uri, imageType, imageView, displayImageOptions, imageSize, listener, progressListener, regionDecodeRect);
    ...
    private void directShowImage(final ImageView imageView, final Drawable drawable) {
        ...
        if (MiscUtil.isValid(uris)) {
            ...
            for (int i = 0; i < uris.size(); i++) {
                if (uris.get(i) == null) {
                    ...
                    list.add(new NonViewAwareImp(uri, downloadType, cloudImageLoadingListener, cloudImageLoadingProgressListener));
                }
            }
        ...
        if (delayMap != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (!(aware == null || isValidRequest(aware.getInternalKey(), aware))) {
                    iterator.remove();
                    onInvalidDownloadItem(aware.getInternalUri(), aware.getInternalDownloadType(), aware.getImageView(), aware.getLoadingListener());
                }
            }
            if (MiscUtil.isValid(validRequests)) {
