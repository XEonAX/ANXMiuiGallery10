...
public class Log2File {
    ...
    private void initialWriter() {
        if (this.mCanLog) {
            try {
                this.mOut = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(StorageUtils.getPathInPrimaryStorage(FileUtils.concat("MIUI/Gallery/cloud", ((SimpleDateFormat) DATE_FORMAT.get()).format(new Date()) + "_gallery_log")), true)));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
