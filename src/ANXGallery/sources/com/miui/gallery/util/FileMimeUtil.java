...
public class FileMimeUtil {
    ...
    private static String rawGetMimeType(String path, Mime[] mimes) throws IOException {
        Throwable th;
        ...
        try {
            ...
            try {
                ...
                if (endBufferedLength < 0 || endBufferedLength > 2147483647L) {
                    Log.e("FileMimeUtil", String.format("unexpected error, endBufferedLength: %d, file length: %d", new Object[]{Long.valueOf(endBufferedLength), Long.valueOf(fileLength)}));
                    MiscUtil.closeSilently(file2);
                    return null;
                } else if (endBufferedLength == 0) {
                    Log.d("FileMimeUtil", "endBufferedLength is 0, just return null mime type");
                    MiscUtil.closeSilently(file2);
                    return null;
                } else {
    ...
    public static String getSupportUploadMimeType(String path) {
        if (!TextUtils.isEmpty(path)) {
            ...
            if (!TextUtils.isEmpty(mimeType) && (isImageFromMimeType(mimeType) || isVideoFromMimeType(mimeType))) {
                ...
                if (lastDot >= 0) {
                    ...
                    if (isVideoFromMimeType(mimeType)) {
                        try {
                            supportUploadMimeType = rawGetMimeType(path, VIDEO_MIMES);
                        } catch (Throwable e) {
                            Log.w("FileMimeUtil", e);
                        }
                        if (supportUploadMimeType != null) {
