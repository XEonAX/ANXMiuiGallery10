...
public class MediaSortDateHelper {
    ...
    private static class CacheBackedSortDateProvider implements SortDateProvider {
        ...
        public SortDate getSortDateByAlbumPath(String albumPath) {
            if (!MiscUtil.isValid(this.mSortByModifyAlbumPaths) || TextUtils.isEmpty(albumPath)) {
        ...
        public List<String> getAlbumPathsBySortDate(SortDate sortDate) {
            if (sortDate == SortDate.MODIFY_TIME && MiscUtil.isValid(this.mSortByModifyAlbumPaths)) {
        ...
        public List<Long> getAlbumIdsBySortDate(SortDate sortDate) {
            if (sortDate == SortDate.MODIFY_TIME && MiscUtil.isValid(this.mSortByModifyAlbumIds)) {
        ...
        private static void updateCacheInfo() {
            ...
            if (MiscUtil.isValid(cachePathSet)) {
                String str = null;
                cacheIdSet = (HashSet) SafeDBUtil.safeQuery(GalleryApp.sGetAndroidContext(), GalleryContract.Album.URI, new String[]{"_id"}, String.format("%s COLLATE NOCASE IN ('%s')", new Object[]{"local_path", TextUtils.join("', '", cachePathSet)}), null, str, new QueryHandler<HashSet<String>>() {
