...
public class GalleryOpenProvider extends ContentProvider {
    private static final String[] CLOUD_COLUMNS = new String[]{"_id", "serverType", "exifGPSLatitude", "exifGPSLatitudeRef", "exifGPSLongitude", "exifGPSLongitudeRef", "dateTaken", "exifOrientation", "localGroupId", "microthumbfile", "thumbnailFile", "localFile", "size", "fileName", SimpleDialogFragment.ARG_TITLE, "dateModified", "mimeType", "exifImageWidth", "exifImageLength", "duration"};
    private static final String[] MEDIA_COLUMNS = new String[]{"_id", "_data", "_size", "_display_name", SimpleDialogFragment.ARG_TITLE, "date_added", "date_modified", "mime_type", nexExportFormat.TAG_FORMAT_WIDTH, nexExportFormat.TAG_FORMAT_HEIGHT, "description", "picasa_id", "isprivate", "latitude", "longitude", "datetaken", "orientation", "mini_thumb_magic", "bucket_id", "bucket_display_name", "bookmark", "album", "artist", "category", "duration", "language", "resolution", "tags", "mini_thumb_data", "media_type"};
    ...
    private class MediaInfo {
        ...
        boolean init(String path) {
            ...
            this.mMimeType = FileMimeUtil.getMimeType(path);
            if (!(FileMimeUtil.isImageFromMimeType(this.mMimeType) || FileMimeUtil.isVideoFromMimeType(this.mMimeType))) {
                this.mMimeType = FileMimeUtil.getMimeTypeByParseFile(path);
                if (!(FileMimeUtil.isImageFromMimeType(this.mMimeType) || FileMimeUtil.isVideoFromMimeType(this.mMimeType))) {
                    return false;
                }
            }
            if (FileMimeUtil.isImageFromMimeType(this.mMimeType)) {
                i = 1;
            }
            this.mType = i;
            ...
            } else if (FileMimeUtil.hasExif(path)) {
                Log.d(GalleryOpenProvider.TAG, "reading exif");
                try {
                    ExifInterface exif = new ExifInterface(path);
                    float[] latLong = new float[2];
                    exif.getLatLong(latLong);
                    this.mLatitude = Double.valueOf((double) latLong[0]);
                    this.mLongitude = Double.valueOf((double) latLong[1]);
                    this.mOrientation = Integer.valueOf(com.miui.gallery3d.exif.ExifInterface.getRotationForOrientationValue((short) exif.getAttributeInt("Orientation", 0)));
                    ...
                    Log.w(GalleryOpenProvider.TAG, e2);
                    return true;
                }
            } else {
        ...
        boolean init(Cursor cursor) {
            ...
            if (!(rawLat == null || refLat == null)) {
                this.mLatitude = Double.valueOf((double) com.miui.gallery3d.exif.ExifInterface.convertRationalLatLonToFloat(rawLat, refLat));
            }
            String rawLon = cursor.getString(4);
            String refLon = cursor.getString(5);
            if (!(rawLon == null || refLon == null)) {
                this.mLongitude = Double.valueOf((double) com.miui.gallery3d.exif.ExifInterface.convertRationalLatLonToFloat(rawLon, refLon));
            }
            ...
            try {
                ...
                if (!TextUtils.isEmpty(string)) {
                    ...
                    if (!cursor.isNull(7)) {
                        this.mOrientation = Integer.valueOf(com.miui.gallery3d.exif.ExifInterface.getRotationForOrientationValue(cursor.getShort(7)));
                    }
                    if (this.mType == 2) {
                        this.mResolution = this.mWidth + "x" + this.mHeight;
                        this.mAlbumName = this.mBucketName;
