...
public class CoverImageSizeStrategy extends BaseStrategy {
    ...
    public boolean isValid(Cursor cursor) {
        ...
        if (DecodeRegionImageUtils.roundToSquareAndScale(new Rect((int) (faceRect.left * ((float) width)), (int) (faceRect.top * ((float) height)), (int) (faceRect.right * ((float) width)), (int) (faceRect.bottom * ((float) height))), width, height, 2.0f).width() >= GalleryApp.sGetAndroidContext().getResources().getDimensionPixelSize(R.dimen.people_face_size_large)) {
            return true;
        }
        return false;
    }
