...
public class GalleryCloudProvider extends ContentProvider {
        ...
        try {
            if ("cloud".equals(table)) {
                ...
                if (queryCursor != null && queryCursor.moveToFirst()) {
                    ...
                    GallerySamplingStatHelper.recordCountEvent("Sync", "sync_insert_transaction", params);
                }
            }
            MiscUtil.closeSilently(queryCursor);
            return id;
        } catch (Throwable th) {
    ...
    private Cursor querySyncInfo(Uri uri, String[] projection, String selection, String[] selectionArgs, String orderBy) {
        ...
        String OWNER_DIRTY_SELECTION = String.format(Locale.US, "(%s) AND (%s = %s OR %s = %s) AND (%s = %s OR %s = %s)", new Object[]{CloudUtils.SELECTION_OWNER_NEED_SYNC, "serverType", Integer.valueOf(1), "serverType", Integer.valueOf(2), "localFlag", Integer.valueOf(7), "localFlag", Integer.valueOf(8)});
        String SHARER_DIRTY_SELECTION = String.format(Locale.US, "(%s = %d OR %s = %d) AND (%s = %s OR %s = %s)", new Object[]{"localFlag", Integer.valueOf(7), "localFlag", Integer.valueOf(8), "serverType", Integer.valueOf(1), "serverType", Integer.valueOf(2)});
