...
public class AlbumManager {
    ...
    private static String SQL_ALBUM_SORT_BY = "CASE WHEN sortBy IS NULL THEN dateTaken ELSE sortBy END AS sortBy";
    private static String SQL_ALBUM_UNWRITEABLE = "CASE WHEN localFile LIKE '/%%' THEN 1 ELSE 0 END AS album_unwriteable";
    ...
    private static String SQL_SHARE_ALBUM_SORT_BY = "CASE WHEN sortBy IS NULL THEN dateTaken ELSE sortBy END AS sortBy";
    ...
    public static Cursor query(Context context, SQLiteDatabase db, String[] projection, String selection, String[] selectionArgs, String orderBy, boolean joinVideoAlbum, boolean joinFaceAlbum, boolean joinShare, boolean joinPano, boolean joinRecent, boolean joinFavorites, boolean excludeEmptyAlbum, boolean queryAllExceptDeleted, boolean removeDuplicateItems, Integer mediaType) {
        String albumCover;
        ...
        if (MiscUtil.isValid(MediaSortDateHelper.getSortDateProvider().getAlbumPathsBySortDate(SortDate.MODIFY_TIME))) {
        ...
        if (joinFaceAlbum && ((mediaType == null || mediaType.intValue() == 1) && isIncludeFaceAlbum(excludeNonLocalAlbums))) {
            sqlJoinFaceAlbum = getFaceAlbumColumns(context);
        }
        if (joinVideoAlbum && (mediaType == null || mediaType.intValue() == 2)) {
            sqlJoinVideoAlbum = getVideoAlbumColumns(context, albumCover, excludeHiddenAlbums, excludeNonLocalAlbums);
        }
        if (joinShare && !excludeNonLocalAlbums) {
            sqlJoinShareAlbum = getShareAlbumBasicColumns(context, excludeHiddenAlbums, removeDuplicateItems, mediaType);
        }
        if (joinPano && (mediaType == null || mediaType.intValue() == 1)) {
            sqlJoinPanoAlbum = getPanoAlbumColumns(context, albumCover, excludeHiddenAlbums, excludeNonLocalAlbums);
        }
        if (joinRecent) {
            sqlJoinRecentAlbum = getRecentAlbumColumns(excludeHiddenAlbums, excludeNonLocalAlbums, removeDuplicateItems, mediaType);
        }
        if (joinFavorites) {
            sqlJoinFavoritesAlbum = getFavoritesAlbumColumns(albumCover, excludeHiddenAlbums, excludeNonLocalAlbums, mediaType);
        }
        String albumBasic = getAlbumBasicColumns(context, excludeHiddenAlbums, excludeNonLocalAlbums);
        ...
        objArr[8] = sqlJoinFaceAlbum;
        objArr[9] = sqlJoinPanoAlbum;
        objArr[10] = sqlJoinRecentAlbum;
        objArr[11] = sqlJoinFavoritesAlbum;
    ...
    private static String getAlbumBasicColumns(Context context, boolean excludeHiddenAlbums, boolean excludeNonLocalAlbums) {
    ...
    private static String getShareAlbumBasicColumns(Context context, boolean excludeHiddenAlbums, boolean removeDuplicateItems, Integer mediaType) {
    ...
    private static String getPanoAlbumColumns(Context context, String albumCover, boolean excludeHiddenAlbums, boolean excludeNonLocalAlbums) {
    ...
    private static String getRecentAlbumColumns(boolean excludeHiddenAlbums, boolean excludeNonLocalAlbums, boolean removeDuplicateItems, Integer mediaType) {
        ...
        objArr[1] = removeDuplicateItems ? ", count(DISTINCT sha1) AS media_count" : ", count(_id) AS media_count";
        objArr[2] = SQL_RECENT_ALBUM_COVER;
        objArr[3] = ", 0 AS size ";
        objArr[4] = getMediaTypeSelection(mediaType);
        objArr[5] = getExcludeNonLocalSelection(excludeNonLocalAlbums);
    ...
    private static String getFavoritesAlbumColumns(String albumCover, boolean excludeHiddenAlbums, boolean excludeNonLocalAlbums, Integer mediaType) {
