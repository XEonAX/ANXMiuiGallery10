...
public class ViewPager extends ViewGroup implements ISlipAnimView {
    ...
    private void calculatePageWidthFactor(ItemInfo curItem) {
        ...
        while (it.hasNext()) {
            ...
            float wRatio = 1.0f;
            if (!MiscUtil.floatEquals(this.mSlipProgress, 0.0f)) {
    ...
    public void setHeightSlipRatio(float ratio) {
        this.mHeightSlipRatio = MiscUtil.clamp(ratio, 0.0f, 1.0f);
        if (isLaidOut()) {
            if (DEBUG) {
    ...
    public void setWidthSlipRatio(float ratio) {
        this.mWidthSlipRatio = MiscUtil.clamp(ratio, 0.0f, 1.0f);
        if (isLaidOut()) {
            if (DEBUG) {
    ...
    public void setMarginSlipRatio(float ratio) {
        this.mMarginSlipRatio = MiscUtil.clamp(ratio, 0.0f, 1.0f);
        if (isLaidOut()) {
            if (DEBUG) {
    ...
    private int findPointerIndex(MotionEvent ev, int activePointerId) {
        ...
        GallerySamplingStatHelper.recordErrorEvent("gesture", "gesture_view_pager", params);
        return -1;
    }
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (pointerIndex != -1) {
                        ...
                        if (!(MiscUtil.floatEquals(dx, 0.0f) || isGutterDrag(this.mLastMotionX, dx))) {
