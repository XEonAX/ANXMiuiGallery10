...
import com.miui.gallery.adapter.BaseMediaAdapter.CheckedItem;
...
public class EditableListViewWrapper {
    ...
    private OnItemClickListener mCheckItemListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
            if ((view instanceof Checkable) && position >= EditableListViewWrapper.this.mListHeaderCount) {
                position -= EditableListViewWrapper.this.mListHeaderCount;
                boolean checked = !EditableListViewWrapper.this.mCheckState.getCheckState(position);
    ...
    private OnItemLongClickListener mEnterActionModeListener = new OnItemLongClickListener() {
        public boolean onItemLongClick(AdapterView<?> adapterView, View view, int position, long id) {
            if (position < EditableListViewWrapper.this.mListHeaderCount) {
                return false;
            }
    ...
    private DataSetObserver mRestoreCheckStateObserver = new DataSetObserver() {
        ...
        public void onInvalidated() {
            super.onInvalidated();
            EditableListViewWrapper.this.handleDataInvalid();
        }
    };
    private OnScrollListener mScrollDelegate = new OnScrollListener() {
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            EditableListViewWrapper.this.mListScrollState = scrollState;
            if (EditableListViewWrapper.this.mScrollListener != null) {
                EditableListViewWrapper.this.mScrollListener.onScrollStateChanged(view, scrollState);
            }
        }

        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            if (EditableListViewWrapper.this.mScrollListener != null) {
                EditableListViewWrapper.this.mScrollListener.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
            }
        }
    };
    ...
    public static class SimpleMultiChoiceModeListener implements MultiChoiceModeListener {
    ...
    class AnimationManager implements OnTouchListener {
        ...
        class Scroll2PickHelper implements OnTouchListener {
                ...
                switch (ev.getActionMasked()) {
                    ...
                    case 3:
                        ...
                        if (!(this.mHasEverPickLongTouchPosition || this.mLongTouchPosition == -1)) {
                            ...
                            View view = ((StickyGridHeadersGridView) EditableListViewWrapper.this.mListView).getChildViewByItemIndex(this.mLongTouchPosition);
                            if (view instanceof BackgroundImageViewable) {
                    ...
                    case 2:
                        ...
                        if (!(EditableListViewWrapper.this.mListScrollState == 0 || interceptBefore || !interceptAfter)) {
                            EditableListViewWrapper.this.mListView.setOnTouchListener(null);
                            EditableListViewWrapper.this.mListView.requestDisallowInterceptTouchEvent(true);
                            MotionEvent fakeEvent = MotionEvent.obtain(ev);
                            fakeEvent.setAction(1);
                            EditableListViewWrapper.this.mListView.dispatchTouchEvent(fakeEvent);
                            EditableListViewWrapper.this.mListView.requestDisallowInterceptTouchEvent(false);
                            EditableListViewWrapper.this.mListView.setOnTouchListener(EditableListViewWrapper.this.mAnimationManager);
                        }
            ...
            private boolean onTouchMove(MotionEvent ev) {
                ...
                if (Math.abs(x - this.mLastX) >= Math.abs(y - this.mLastY) && Math.abs(x - this.mLastX) > 10.0f) {
                    position = EditableListViewWrapper.this.mListView.pointToPosition((int) x, (int) y);
                    if (!(position == -1 || position == this.mLastPosition)) {
                        boolean z;
                        int positionExcludeHeader = ((StickyGridHeadersGridView) EditableListViewWrapper.this.mListView).getItemIndexByItemPosition(position);
                        if (EditableListViewWrapper.this.mCheckState.getCheckState(positionExcludeHeader)) {
                            z = false;
                        } else {
                            z = true;
                        }
            ...
            private void ifScrollYModeEnable(float x, float y) {
                ...
                if (!this.mScollYMode2PickEnable) {
                    ...
                    if (this.mScollYMode2PickEnable) {
                        this.mMotionPosition = EditableListViewWrapper.this.mListView.pointToPosition((int) x, (int) y);
            ...
            private boolean pickOrNotByPosition(int position, boolean checked) {
                View view = EditableListViewWrapper.this.mListView.getChildAt(position - EditableListViewWrapper.this.mListView.getFirstVisiblePosition());
                if (!(view instanceof BackgroundImageViewable)) {
                    return false;
                }
                ...
                EditableListViewWrapper.this.mCheckState.setCheckState(positionExcludeHeader, checked);
                ((Checkable) view).setChecked(checked);
                return true;
            }
        }
        ...
        public void startEnterActionModeAni() {
            startScaleListViewAnimation(EditableListViewWrapper.this.mListView, false);
        }

        public void startExistActionModeAni() {
            startScaleListViewAnimation(EditableListViewWrapper.this.mListView, true);
        }
        ...
        public void resetViewPropertyIfNeed(View view, View convertView, int position) {
            ...
            if (this.isTurnOnScaleImageViewAni && (view instanceof BackgroundImageViewable)) {
                ImageView imageView = ((BackgroundImageViewable) view).getBackgroundImageView();
                if (EditableListViewWrapper.this.isInChoiceMode()) {
                    if (!(imageView.getTag(R.id.tag_pick_position) == null || ((Integer) imageView.getTag(R.id.tag_pick_position)).intValue() == position || EditableListViewWrapper.this.mCheckState.getCheckState(position))) {
        ...
        public boolean onTouch(View v, MotionEvent ev) {
            if (EditableListViewWrapper.this.mListView instanceof StickyGridHeadersGridView) {
    ...
    private class SimpleWrapper implements ListAdapter, Source {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (view instanceof Checkable) {
                ...
                EditableListViewWrapper.this.mAnimationManager.resetViewPropertyIfNeed(view, convertView, position);
                return view;
            }
    ...
    private class SimpleTimeLineWrapper extends SimpleWrapper implements StickyGridHeadersSimpleAdapter {
        ...
        public View getHeaderView(int position, View convertView, ViewGroup parent) {
            ...
            if (view instanceof CheckableView) {
                EditableListViewWrapper.this.setCheckableHeaderView((CheckableView) view, ((StickyGridHeadersGridView) EditableListViewWrapper.this.mListView).getGroupIndexByItemIndex(position));
            }
            return view;
        }
    ...
    private class TimeLineWrapper extends SimpleWrapper implements StickyGridHeadersBaseAdapter {
        ...
        public View getHeaderView(int position, View convertView, ViewGroup parent) {
            View view = this.mWrapped.getHeaderView(position, convertView, parent);
            if (view instanceof CheckableView) {
                EditableListViewWrapper.this.setCheckableHeaderView((CheckableView) view, position);
            }
            return view;
        }
    ...
    public EditableListViewWrapper(AbsListView listView) {
        if (listView == null) {
    ...
    public List<CheckedItem> getCheckedItems() {
        ...
        if (this.mAdapterWrapper != null && (this.mAdapterWrapper.getWrapped() instanceof BaseMediaAdapter)) {
            BaseMediaAdapter originalAdapter = (BaseMediaAdapter) this.mAdapterWrapper.getWrapped();
        ...
        if (isInChoiceMode()) {
            checkableView.setCheckableListener(new OnClickListener() {
                public void onClick(View v) {
                    EditableListViewWrapper.this.setGroupCheck(position);
