...
public class ScreenshotScanner extends BaseScanner {
    ...
    private int queryScreenshotAlbumId(Context context) {
        return ((Integer) SafeDBUtil.safeQuery(context, Album.URI, new String[]{"_id"}, "serverId = 2", null, null, new QueryHandler<Integer>() {
            ...
            while (it.hasNext()) {
                ...
                if (count < paths.length) {
                    ...
                    if (TextUtils.isEmpty(paths[count])) {
                        paths[count] = FileUtils.concat(StorageUtils.getPriorMicroThumbnailDirectory(), CloudUtils.getThumbnailNameBySha1(item.mSha1));
                    }
                }
                count++;
            }
