...
public class MediaScannerUtil {
    ...
    private static void addRelativePathsToScanList(Context context, ArrayList<String> relativePaths, ArrayList<String> scanList, ArrayList<Integer> bucketIds) {
        if (MiscUtil.isValid(relativePaths)) {
    ...
    private static void scanAbsolutePaths(Context context, ArrayList<String> paths, int priority, boolean cancelRunning) {
        if (MiscUtil.isValid(paths)) {
            ...
            context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_folder_paths", cacheKey).putExtra("key_scan_priority", priority).putExtra("key_cancel_running", cancelRunning).putExtra("key_bulk_notify", true));
        }
    }
    ...
    public static void scanAllAlbumDirectories(Context context, int priority, boolean cancelRunning) {
        if (context != null && checkStoragePermission(context)) {
            ...
            if (MiscUtil.isValid(externalImages)) {
                scanList.addAll(externalImages);
            }
            if (MiscUtil.isValid(externalVideo)) {
    ...
    public static void scanDirectories(Context context, ArrayList<String> paths, int priority, boolean bulkNotify, boolean recursiveScan) {
        if (MiscUtil.isValid(paths) && checkStoragePermission(context)) {
            String cacheKey = String.valueOf(SystemClock.elapsedRealtimeNanos());
            GalleryDataCache.getInstance().put(cacheKey, paths);
            context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_folder_paths", cacheKey).putExtra("key_scan_priority", priority).putExtra("key_bulk_notify", bulkNotify).putExtra("key_recursive_scan", recursiveScan));
        }
    }
    ...
    public static void scanSingleFile(Context context, String path, int priority, boolean forceScan) {
        if (context != null && !TextUtils.isEmpty(path) && checkStoragePermission(context)) {
            context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_file_path", path).putExtra("key_scan_priority", priority).putExtra("key_force_scan", forceScan));
        }
    }
    ...
    public static void scanVolume(Context context, String volume) {
        if (context != null && !TextUtils.isEmpty(volume) && checkStoragePermission(context)) {
            context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_volume", volume));
        }
    }
    ...
    public static void scanMediaProvider(Context context) {
        if (context == null) {
            ...
            do {
                ...
                context2 = context;
                ArrayMap<Integer, String> externalImages = (ArrayMap) SafeDBUtil.safeQuery(context2, Media.getContentUri("external"), FIND_NEW_MEDIA_PROJECTION, String.format(Locale.US, "0=0) AND date_added >= %s / 1000  AND _id < %s ORDER BY _id DESC  LIMIT (2000", new Object[]{String.valueOf(lastImagesScanTime), String.valueOf(lastMinId)}), null, null, new QueryHandler<ArrayMap<Integer, String>>() {
                ...
                if (externalImages == null || externalImages.size() <= 0) {
                    lastVideosScanTime = MediaScanner.getLastVideosScanTime();
                    lastMinId = Integer.MAX_VALUE;
                } else {
                    lastQueryCount = externalImages.size();
                    lastMinId = ((Integer) Collections.min(externalImages.keySet())).intValue();
                    try {
                        ...
                        context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_files_path", cacheKey).putExtra("key_scan_priority", 2).putExtra("key_scan_type", 1).putExtra("key_task_start_time", taskStartTime));
                    } catch (Throwable e) {
            ...
            } while (lastQueryCount >= 2000);
            lastVideosScanTime = MediaScanner.getLastVideosScanTime();
            lastMinId = Integer.MAX_VALUE;
            do {
                taskStartTime = System.currentTimeMillis();
                context2 = context;
                ArrayMap<Integer, String> externalVideos = (ArrayMap) SafeDBUtil.safeQuery(context2, Video.Media.getContentUri("external"), FIND_NEW_MEDIA_PROJECTION, String.format(Locale.US, "0=0) AND date_added >= %s / 1000  AND _id < %s ORDER BY _id DESC  LIMIT (2000", new Object[]{String.valueOf(lastVideosScanTime), String.valueOf(lastMinId)}), null, null, new QueryHandler<ArrayMap<Integer, String>>() {
                    public ArrayMap<Integer, String> handle(Cursor cursor) {
                ...
                if (externalVideos != null && externalVideos.size() > 0) {
                    lastQueryCount = externalVideos.size();
                    lastMinId = ((Integer) Collections.min(externalVideos.keySet())).intValue();
                    try {
                        data = new ArrayList(externalVideos.values());
                        cacheKey = String.valueOf(SystemClock.elapsedRealtimeNanos());
                        GalleryDataCache.getInstance().put(cacheKey, data);
                        context.startService(new Intent(context, MediaScannerService.class).putExtra("scan_files_path", cacheKey).putExtra("key_scan_priority", 2).putExtra("key_scan_type", 2).putExtra("key_task_start_time", taskStartTime));
                    } catch (Throwable e2) {
    ...
    private static void recordScannerException(String method, Exception e) {
        ...
        GallerySamplingStatHelper.recordErrorEvent("media_scanner", String.format(Locale.US, "scanner_%s", new Object[]{e.getClass().getSimpleName()}), params);
    }

    public static void cleanup(Context context) {
        if (context != null && checkStoragePermission(context)) {
            context.startService(new Intent(context, MediaScannerService.class).putExtra("key_trigger_cleanup", true));
        }
    }
