...
public class SaveToCloudUtil {
    ...
    private static int checkFileSize(SaveToCloud saveToCloud) {
        ...
        if (saveToCloud.mSize > CloudUtils.getMaxFileSizeLimit(saveToCloud.mMimeType)) {
            return -5;
        }
        return 0;
    }
    ...
    private static int prepareInsert(Context context, SaveToCloud saveToCloud, SaveParams saveParams, ContentProviderBatchOperator batchOperator) {
            ...
            try {
                ...
                if (MiscUtil.isValid(entries)) {
                    ...
                    if (conflictEntry != null) {
                        ...
                        GallerySamplingStatHelper.recordCountEvent("media_scanner", "scanner_rename_conflict_thumbnail", params);
        ...
        try {
            ...
            if (MiscUtil.isValid(entries)) {
                ...
                while (it.hasNext()) {
                    ...
                    if (saveToCloud.mSha1.equalsIgnoreCase(entry.mSha1)) {
                        ...
                        GallerySamplingStatHelper.recordCountEvent("media_scanner", "file_content_changed", param);
                        return -7;
                    } else if (saveToCloud.mName.equalsIgnoreCase(entry.mName)) {
                        conflictEntry = entry;
                    }
                ...
                if (needCheckExifSha1(saveToCloud.mName) && saveToCloud.mSize < 1048576) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (!TextUtils.isEmpty(entry.mThumbnailPath) && entry.mThumbnailPath.endsWith(saveToCloud.mName)) {
                            ...
                            GallerySamplingStatHelper.recordCountEvent("media_scanner", "treat_as_thumbnail", param);
                            return -2;
                        }
                    }
                }
            ...
            if (!saveParams.isInternalScanRequest()) {
                ...
                if (streamFile == null || !streamFile.isStreamFolder(relativeFolder)) {
                    if (saveToCloud.mLastModify - new File(parentFolder).lastModified() > 1000) {
                        ...
                        GallerySamplingStatHelper.recordErrorEvent("media_scanner", "scanner_stream_file", params);
                    }
            ...
            if (conflictEntry != null) {
                ...
                GallerySamplingStatHelper.recordCountEvent("media_scanner", "scanner_rename_conflict_origin", params);
                scanRenamePath = CloudUtils.renameForPhotoConflict(scanPath);
    ...
    private static void putValuesForImage(String path, ContentValues values, long lastModified) {
        try {
            ...
            if (FileMimeUtil.hasExif(path)) {
                ...
                values.put("specialTypeFlags", Long.valueOf(SpecialTypeMediaUtils.parseFlagsForImage(path)));
    ...
    private static void statVideoInfo(Context context, long duration, String path, long size) {
        if (duration > 0) {
            ...
            GallerySamplingStatHelper.recordCountEvent("media_scanner", "video_duration", params);
        }
    }
        ...
        SafeDBUtil.safeQuery(context, isImage ? Media.EXTERNAL_CONTENT_URI : Video.Media.EXTERNAL_CONTENT_URI, isImage ? ENSURE_INFO_PROJECTION_IMAGE : ENSURE_INFO_PROJECTION_VIDEO, "_data=?", new String[]{filePath}, null, new QueryHandler() {
            public Object handle(Cursor cursor) {
                if (cursor != null && cursor.moveToNext()) {
                    ...
                    if (!MiscUtil.doubleEquals(latitude, 0.0d) && values.get("exifGPSLatitude") == null) {
                        values.put("exifGPSLatitude", ExifUtil.convertDoubleToLaLon(latitude));
                    }
                    if (!MiscUtil.doubleEquals(longitude, 0.0d) && values.get("exifGPSLongitude") == null) {
                        values.put("exifGPSLongitude", ExifUtil.convertDoubleToLaLon(longitude));
                    }
                }
                return null;
            }
