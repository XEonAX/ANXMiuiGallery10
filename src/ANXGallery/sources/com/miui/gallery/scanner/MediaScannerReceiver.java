        ...
        if (!intent.getBooleanExtra("com.miui.gallery.extra.trigger_scan", false)) {
            ...
            if ("android.intent.action.MEDIA_SCANNER_FINISHED".equals(action)) {
                ...
                if (!TextUtils.isEmpty(path)) {
                    MediaScannerUtil.scanDirectory(context.getApplicationContext(), path, 5, true, true);
                }
            }
        }
    }
            ...
            public Object run(JobContext jc) {
                if (uri != null && "file".equals(uri.getScheme())) {
                    ...
                    if (!TextUtils.isEmpty(path) && new File(path).isDirectory()) {
                        MediaScannerUtil.scanDirectory(context.getApplicationContext(), path, 5, true, false);
                    }
                }
                return null;
            }
