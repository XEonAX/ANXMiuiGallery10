...
public class CollageLayout extends ViewGroup {
    ...
    private static class DragBitmapItem {
        ...
        void show(AnimatorUpdateListener animatorUpdateListener) {
            ...
            r0[1] = PropertyValuesHolder.ofInt("alpha", new int[]{this.mPaint.getAlpha(), 200});
        ...
        ObjectAnimator getTransitionAnimator(RectF target, boolean resetUserMatrix, AnimatorUpdateListener animatorUpdateListener) {
            ...
            if (resetUserMatrix) {
                ...
                propertyValuesHolderArr[1] = PropertyValuesHolder.ofInt("alpha", new int[]{this.mPaint.getAlpha(), 255});
    ...
    private class PopupListener implements ControlListener {
        ...
        public void onReplace() {
            if (CollageLayout.this.mReplaceImageListener != null) {
                CollageLayout.this.mReplaceImageListener.onReplace(CollageLayout.this.mActiveIndex);
            }
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (this.mActiveIndex != -1) {
            ...
            int strokeWidth = (this.mMargin * 2) + this.mActiveLineWidth;
            this.mPaint.setStrokeWidth((float) (this.mActiveLineWidthWhite + strokeWidth));
            this.mPaint.setColor(-1);
            canvas.drawPath(layoutParams.mImageLocation.getPathForChoose(), this.mPaint);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                this.mImageLocationProcessor.processorImageLocation(lp.mImageLocation, (float) width, (float) height, (float) this.mMargin);
    ...
    public void setCollageMargin(float margin) {
        int marginInt = Math.round(this.mDensity * margin);
        if (this.mMargin != marginInt) {
            this.mMargin = marginInt;
            requestLayout();
        }
    }
    ...
    public void setSingleViewBitmap(int index, Bitmap bitmap) {
        if (index >= 0 && index < getChildCount()) {
            ((CollageImageView) getChildAt(index)).setBitmap(bitmap);
        }
    }
