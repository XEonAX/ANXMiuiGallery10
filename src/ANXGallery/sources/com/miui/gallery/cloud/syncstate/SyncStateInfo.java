...
public class SyncStateInfo {
    ...
    public int[] getDirtyCount() {
        return new int[]{this.mImageDirtyCount, this.mVideoDirtyCount};
    }
    ...
    private SyncStatus updateSyncStatusInternal(Context context) {
        ...
        if (!checkIsSyncingMetaData(account) || hasFirstSynced) {
            ...
            tryRefreshSyncType();
            if (this.mImageDirtyCount == 0 && this.mVideoDirtyCount == 0) {
    ...
    private void updateDirtyCount(Context context) {
        int[] dirtyCount = SyncStateUtil.getDirtyCount(context);
        if (this.mImageDirtyCount != dirtyCount[0] || this.mVideoDirtyCount != dirtyCount[1]) {
            this.mImageDirtyCount = dirtyCount[0];
            this.mVideoDirtyCount = dirtyCount[1];
