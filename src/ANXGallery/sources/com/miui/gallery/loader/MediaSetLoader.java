...
public class MediaSetLoader extends CursorSetLoader {
    ...
    private class MediaDataSet extends CursorDataSet {
        ...
        protected void bindItem(BaseDataItem item, int position) {
            if (isValidate(position)) {
                ...
                item.setKey(this.mCursor.getLong(0)).setFilePath(path).setThumbPath(path).setSize(this.mCursor.getLong(2)).setMimeType(this.mCursor.getString(3)).setLatitude(this.mCursor.getFloat(4)).setLongitude(this.mCursor.getFloat(5)).setCreateTime(this.mCursor.getLong(6)).setDuration(this.mCursor.getInt(7) / 1000).setWidth(this.mCursor.getInt(8)).setHeight(this.mCursor.getInt(9));
            }
        }
        ...
        protected String getDeleteDialogMessage(BaseDataItem item, Context context) {
            if (!isFromCamera() || !SyncUtil.existXiaomiAccount(MediaSetLoader.this.getContext())) {
                return super.getDeleteDialogMessage(item, context);
            }
            return context.getResources().getQuantityString(FileMimeUtil.isVideoFromMimeType(item.getMimeType()) ? R.plurals.delete_video_from_all_devices_and_cloud_msg : R.plurals.delete_photo_from_all_devices_and_cloud_msg, 1, new Object[]{Integer.valueOf(1)});
        }
        ...
        protected int doDelete(BaseDataItem item) {
            ...
            if (item != null) {
                ...
                if (rows > 0) {
                    MediaAndAlbumOperations.deleteInService(MediaSetLoader.this.getContext(), isFromCamera() ? 0 : 1, 30, item.getOriginalPath());
    ...
    protected String getSelection() {
        ...
        return "bucket_id in (" + TextUtils.join(",", this.mBucketIds) + ")" + " AND (" + "media_type" + " in (" + 1 + "," + 3 + ")" + (MiscUtil.isValid(this.mProcessingIds) ? " OR _id in (" + TextUtils.join(",", this.mProcessingIds) + ")" : "") + ")";
    }
