...
public class StorageSetLoader extends BaseLoader {
    ...
    private class StorageDataSet extends BaseDataSet {
        ...
        protected void bindItem(BaseDataItem item, int position) {
            ...
            if (file != null) {
                ...
                item.setKey((long) file.getName().hashCode()).setFilePath(path).setThumbPath(path).setMimeType(FileMimeUtil.getMimeType(path));
            }
        }
    ...
    private boolean isSupportType(String mimeType) {
        return FileMimeUtil.isImageFromMimeType(mimeType) || FileMimeUtil.isVideoFromMimeType(mimeType);
    }

    private boolean isSupportFile(String path) {
        boolean support = isSupportType(FileMimeUtil.getMimeType(path));
        if (support || !path.equalsIgnoreCase(this.mInitFilePath)) {
            return support;
        }
        return support | isSupportType(FileMimeUtil.getMimeTypeByParseFile(path));
    }
}
