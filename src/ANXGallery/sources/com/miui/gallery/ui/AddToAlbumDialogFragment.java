...
public class AddToAlbumDialogFragment extends GalleryDialogFragment {
    ...
    private String getSelection() {
        ...
        boolean showUnWritableAlbum = false;
        Bundle arguments = getArguments();
        if (arguments != null) {
            if (arguments.containsKey("show_system_album")) {
                showSystemAlbum = arguments.getBoolean("show_system_album");
            }
            showUnWritableAlbum = arguments.getBoolean("show_unwritable_album");
        }
        String notShowSystemAlbumClause = Cloud.ALIAS_NON_SYSTEM_ALBUM;
        String notShowUnWritableAlbumClause = "album_unwriteable = 0";
        String onlyShowLocalAlbumClause = isOnlyShowLocal() ? " AND (media_count>0)" : "";
        if (!showSystemAlbum && !showUnWritableAlbum) {
            return notShowSystemAlbumClause + " AND " + notShowUnWritableAlbumClause + onlyShowLocalAlbumClause;
        }
        if (!showSystemAlbum) {
            return notShowSystemAlbumClause + onlyShowLocalAlbumClause;
        }
        if (showUnWritableAlbum) {
            return null;
        }
        return notShowUnWritableAlbumClause + onlyShowLocalAlbumClause;
    }
