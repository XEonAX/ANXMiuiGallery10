...
public class BackupDetailFragment extends PhotoListFragmentBase implements OnSyncStateChangeObserver, UploadStatusChangedListener {
    ...
    static class BackupDetailAdapter extends AlbumDetailSimpleAdapter {
        ...
        public BackupDetailAdapter(Context context, SyncStateShowScene type, int syncStateDisplayOptions) {
    ...
    public View onInflateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        this.mBackupDetailAdapter = new BackupDetailAdapter(this.mActivity, SyncStateShowScene.SCENE_ALWAYS, getSyncStateDisplayOptions());
    ...
    protected OnItemClickListener getGridViewOnItemClickListener() {
        return new OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                ...
                IntentUtil.gotoPhotoPage(BackupDetailFragment.this, adapterView, BackupDetailFragment.this.getUri(), position, BackupDetailFragment.this.mBackupDetailAdapter.getCount() - (BackupDetailFragment.this.mBackupDetailAdapter.getSecretCount() > 0 ? 1 : 0), "(" + BackupDetailFragment.this.getSelection() + ") AND (" + "localGroupId" + " != " + -1000 + ")", BackupDetailFragment.this.getSelectionArgs(), BackupDetailFragment.this.getCurrentSortOrder(), new ImageLoadParams(BackupDetailFragment.this.mBackupDetailAdapter.getItemKey(position), BackupDetailFragment.this.mBackupDetailAdapter.getLocalPath(position), ThumbConfig.get().sMicroTargetSize, BackupDetailFragment.this.mBackupDetailAdapter.getItemDecodeRectF(), position, BackupDetailFragment.this.mBackupDetailAdapter.getMimeType(position), BackupDetailFragment.this.mBackupDetailAdapter.getItemSecretKey(position), BackupDetailFragment.this.mBackupDetailAdapter.getFileLength(position)), BackupDetailFragment.this.mAlbumId, BackupDetailFragment.this.mAlbumName, BackupDetailFragment.this.getSupportOperationMask());
                GallerySamplingStatHelper.recordNumericPropertyEvent(BackupDetailFragment.this.getPageName(), "click_micro_thumb", (long) position);
            }
        };
    }
}
