...
import com.miui.gallery.widget.EditableListViewWrapper.PickAnimationHelper.BackgroundImageViewable;
import com.miui.gallery.widget.EditableListViewWrapper.PickAnimationHelper.ShowNumberWhenPicking;
...
public abstract class MicroThumbGridItem extends RelativeLayout implements CheckableView, BackgroundImageViewable, ShowNumberWhenPicking {
    ...
    public void bindImage(String localPath, Uri downloadUri, DisplayImageOptions displayImageOptions) {
        bindImage(localPath, downloadUri, DownloadType.MICRO, displayImageOptions);
    }
    ...
    public void bindIndicator(String mimeType, long duration, long specialTypeFlags) {
        ...
        if (FileMimeUtil.isGifFromMimeType(mimeType)) {
            visibility = 0;
            indicatorText = "GIF";
        } else if (FileMimeUtil.isVideoFromMimeType(mimeType)) {
            visibility = 0;
    ...
    private boolean needShowSyncIndicator(SyncStateShowScene scene) {
        if (scene == SyncStateShowScene.SCENE_ALWAYS) {
            return true;
        }
        if (scene == SyncStateShowScene.SCENE_IN_CHECK_MODE && isInCheckMode()) {
            return true;
        }
        return false;
    }
    ...
    public void bindSyncIndicator(long id, int newState, SyncStateShowScene showScene) {
        bindSyncIndicator(id, newState, showScene, 7);
    }

    public void bindSyncIndicator(long id, int newState, SyncStateShowScene showScene, int displayLogic) {
