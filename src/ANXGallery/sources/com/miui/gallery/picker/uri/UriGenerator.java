...
public class UriGenerator {
    ...
    private OnClickListener mContinueDownloadListener = new OnClickListener() {
        public void onClick(DialogInterface dialog, int which) {
            Activity activity = (Activity) UriGenerator.this.mActivityWeakReference.get();
            if (UriGenerator.this.mProgressDialog != null && activity != null) {
                UriGenerator.this.mProgressDialog.showAllowingStateLoss(activity.getFragmentManager(), "prepare_progress_dialog");
            }
        }
    };
    ...
    private class OriginUrlRequestProgressListener implements ProgressListener {
        ...
        public void onStart(int totalCount) {
            Activity activity = (Activity) UriGenerator.this.mActivityWeakReference.get();
            if (activity != null) {
                if (UriGenerator.this.mProgressDialog == null) {
                    UriGenerator.this.mProgressDialog = new PrepareProgressDialog();
                }
                UriGenerator.this.mProgressDialog.setMax(totalCount);
                UriGenerator.this.mProgressDialog.setStage(1);
                UriGenerator.this.mProgressDialog.setCancelListener(UriGenerator.this.mCancelOriginInfoListener);
                UriGenerator.this.mProgressDialog.showAllowingStateLoss(activity.getFragmentManager(), "prepare_progress_dialog");
            }
        }
        ...
        public void onEnd(ArrayList<OriginInfo> results, boolean success) {
            finish();
            if (MiscUtil.isValid(results) && success) {
        ...
        public void onUpdate(int progress) {
            if (UriGenerator.this.mProgressDialog != null) {
                UriGenerator.this.mProgressDialog.updateProgress(progress);
            }
        }
        ...
        private void finish() {
            if (UriGenerator.this.mProgressDialog != null) {
                Dialog dialog = UriGenerator.this.mProgressDialog.getDialog();
                if (dialog != null && dialog.isShowing()) {
                    UriGenerator.this.mProgressDialog.dismissAllowingStateLoss();
                }
                UriGenerator.this.mProgressDialog = null;
            }
    ...
    private void deliverResult(List<OriginInfo> results) {
        if (this.mResults.length == 0) {
            this.mUriGenerateListener.onFail();
        } else if (this.mImageType != ImageType.ORIGIN_OR_DOWNLOAD_INFO || MiscUtil.isValid(results)) {
    ...
    private void startDownload() {
        ...
        this.mDownloader = new Downloader(this.mDownloadPendings, new DownloadListener() {
            public void onStart(List<DownloadTask> tasks) {
                ...
                if (activity != null) {
                    UriGenerator.this.mProgressDialog = new PrepareProgressDialog();
                    UriGenerator.this.mProgressDialog.setMax(tasks.size());
                    UriGenerator.this.mProgressDialog.setStage(0);
                    UriGenerator.this.mProgressDialog.setCancelListener(UriGenerator.this.mCancelConfirmListener);
                    UriGenerator.this.mProgressDialog.showAllowingStateLoss(activity.getFragmentManager(), "prepare_progress_dialog");
                }
            }

            public void onUpdate(List<DownloadResult> success, List<DownloadResult> fails) {
                UriGenerator.this.mProgressDialog.updateProgress(success.size() + fails.size());
            }
