...
public class PickGalleryActivity extends PickerActivity {
    ...
    private OnClickListener mContinueDownloadListener = new OnClickListener() {
        public void onClick(DialogInterface dialog, int which) {
            if (PickGalleryActivity.this.mProgressDialog != null) {
                PickGalleryActivity.this.mProgressDialog.showAllowingStateLoss(PickGalleryActivity.this.getFragmentManager(), "prepare_progress_dialog");
            }
        }
    };
    ...
    private PrepareProgressDialog mProgressDialog;
    ...
    private class DownloadProgressListener implements DownloadListener {
        ...
        public void onStart(List<DownloadTask> tasks) {
            ...
            PickGalleryActivity.this.mProgressDialog = new PrepareProgressDialog();
            PickGalleryActivity.this.mProgressDialog.setMax(tasks.size());
            PickGalleryActivity.this.mProgressDialog.setStage(0);
            PickGalleryActivity.this.mProgressDialog.showAllowingStateLoss(PickGalleryActivity.this.getFragmentManager(), "prepare_progress_dialog");
        }

        public void onUpdate(List<DownloadResult> success, List<DownloadResult> fails) {
            PickGalleryActivity.this.mProgressDialog.updateProgress(success.size() + fails.size());
        }
        ...
        public void onEnd(List<DownloadResult> success, List<DownloadResult> fails) {
            ...
            if (PickGalleryActivity.this.mProgressDialog != null) {
                Dialog dialog = PickGalleryActivity.this.mProgressDialog.getDialog();
                if (dialog != null && dialog.isShowing()) {
                    PickGalleryActivity.this.mProgressDialog.dismissAllowingStateLoss();
                }
                ...
                PickGalleryActivity.this.mProgressDialog = null;
            }
    ...
    private class OriginUrlRequestProgressListener implements ProgressListener {
        ...
        public void onStart(int totalCount) {
            ...
            PickGalleryActivity.this.mProgressDialog.setMax(totalCount);
            PickGalleryActivity.this.mProgressDialog.setStage(1);
            PickGalleryActivity.this.mProgressDialog.showAllowingStateLoss(PickGalleryActivity.this.getFragmentManager(), "prepare_progress_dialog");
        }
        ...
        public void onUpdate(int progress) {
            if (PickGalleryActivity.this.mProgressDialog != null) {
                PickGalleryActivity.this.mProgressDialog.updateProgress(progress);
            }
        }
            ...
            if (!isCrossUserPick()) {
                ThreadManager.getMiscPool().submit(new Job<Object>() {
                    public Object run(JobContext jc) {
                        MediaScannerUtil.scanAllAlbumDirectories(PickGalleryActivity.this.getApplicationContext(), 4);
                        return null;
                    }
                });
            }
        ...
        this.mActionBar.addOnFragmentViewPagerChangeListener(new FragmentViewPagerChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                if (PickGalleryActivity.this.mCurrentPagePosition != position) {
                    ...
                    GallerySamplingStatHelper.recordPageStart(PickGalleryActivity.this, PickGalleryActivity.this.getPageName(position));
                }
    ...
    private void prepareResult(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (imageType == ImageType.ORIGIN) {
                ...
                this.mDownloadPendings.add(new DownloadTask(CloudUriAdapter.getDownloadUri(cursor.getLong(0)), downloadType, cursor.getInt(6), position));
            } else {
                if (imageType == ImageType.ORIGIN_OR_DOWNLOAD_INFO) {
                    this.mOriginRequestPendings.add(new OriginUrlRequestTask(position, CloudUriAdapter.getDownloadUri(cursor.getLong(0)), cursor.getInt(7), cursor.getInt(8), cursor.getInt(5)));
                }
                ...
                this.mDownloadPendings.add(new DownloadTask(CloudUriAdapter.getDownloadUri(cursor.getLong(0)), DownloadType.THUMBNAIL, 0, position));
            }
        }
    }
    ...
    private void parseSingle(Intent data, String path) {
        ...
        String mimeType = FileMimeUtil.getMimeType(path);
        if ("*/*".equals(mimeType)) {
            data.setData(result);
        } else {
