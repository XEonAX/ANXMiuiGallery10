...
public class DownloadUtil {
        ...
        if (file.exists() && file.isFile()) {
            ...
            if (file.length() < dbImage.getSize()) {
                ...
                if (report) {
                    ...
                    GallerySamplingStatHelper.recordErrorEvent(String.format("file_download_%s", new Object[]{"origin"}), "file_download_wrong_size", params);
                }
                return false;
            }
            ...
            if (report) {
                ...
                GallerySamplingStatHelper.recordErrorEvent(String.format("file_download_%s", new Object[]{"origin"}), "file_download_wrong_sha1", params);
            }
            return false;
        }
    ...
    public static String getDownloadTempFilePath(DBImage dbImage, DownloadType downloadType) {
        ...
        if (isMicro(downloadType)) {
            return FileUtils.concat(StorageUtils.getMicroThumbnailTempDirectory(), dbImage.getFileName() + ".temp");
        }
        if (isThumbnail(downloadType)) {
            return FileUtils.concat(StorageUtils.getThumbnailTempDirectory(), dbImage.getSha1() + "." + dbImage.getId());
        }
        if (isOrigin(downloadType)) {
            return FileUtils.concat(StorageUtils.getOriginTempDirectory(), dbImage.getSha1() + "." + dbImage.getId());
        }
        return null;
    }
    ...
    public static String getDownloadFilePath(DBImage dbImage, DownloadType downloadType) {
        ...
        String filePath = FileUtils.concat(getDownloadFolderPath(dbImage, downloadType), fileName);
    ...
    public static void statDownloadError(RequestItem item, String url, DownloadFailReason reason) {
        ...
        GallerySamplingStatHelper.recordCountEvent(getStatCategory(item), "file_download_status", params);
    }
    ...
    public static void statDownloadRetryTimes(RequestItem item, int times, int maxTimes) {
        ...
        GallerySamplingStatHelper.recordCountEvent(getStatCategory(item), event, params);
    }
