...
public class DraftManager {
    private AsyncTask<Void, Void, Bitmap> mBackgroundTask = new AsyncTask<Void, Void, Bitmap>() {
        protected Bitmap doInBackground(Void... params) {
            ...
            Bitmap bitmap = null;
            try {
                return DraftManager.this.decodeBitmap(opt, DraftManager.this.mExif);
            } catch (Throwable e) {
    ...
    public boolean initializeForPreview(boolean inMainProcess) throws FileNotFoundException, SecurityException {
        ...
        if (this.mPreview == null) {
            ...
            this.mPreview = decodeBitmap(opt, this.mExif);
            this.mPreviewEnable = true;
        }
    ...
    private Bitmap decodeBitmap(Options opt, ExifInterface exif) throws FileNotFoundException {
        return Bitmaps.setConfig(Bitmaps.joinExif(Bitmaps.decodeStream(getInputStream(), opt), exif, opt));
    }
    ...
    public boolean export(Uri out) {
        ...
        if (tags == null || tags.isEmpty()) {
            ...
            for (ExifTag tag : tags) {
                ...
                if (tagId == ExifInterface.getTrueTagKey(ExifInterface.TAG_ORIENTATION) || tagId == ExifInterface.getTrueTagKey(ExifInterface.TAG_IMAGE_WIDTH) || tagId == ExifInterface.getTrueTagKey(ExifInterface.TAG_IMAGE_LENGTH) || tagId == ExifInterface.getTrueTagKey(ExifInterface.TAG_XIAOMI_COMMENT) || tagId == ExifInterface.getTrueTagKey(ExifInterface.TAG_USER_COMMENT) || tagId == (short) -30576) {
    ...
    private void initForBitmapInfo() throws FileNotFoundException {
        ...
        if (isSecret()) {
            checkSecretInfo();
            this.mExif = ExifUtil.parseExifInfo(this.mSource.getPath(), this.mSecretInfo.mSecretKey);
        } else {
            this.mExif = Bitmaps.readExif(getInputStream());
        }
        Options opt = new Options();
        opt.inJustDecodeBounds = true;
        Bitmaps.decodeStream(getInputStream(), opt);
        Bitmaps.joinExif(null, this.mExif, opt);
    ...
    public Bitmap decodeOrigin() {
        try {
            ...
            return decodeBitmap(opt, this.mExif);
        } catch (Throwable e) {
            Log.w("DraftManager", e);
        } catch (Throwable e2) {
            Log.w("DraftManager", e2);
