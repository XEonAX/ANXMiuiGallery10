...
public class AlbumDetailSimpleAdapter extends PreloadMediaAdapter implements CheckableAdapter {
    ...
    public AlbumDetailSimpleAdapter(Context context) {
        super(context, SyncStateShowScene.SCENE_IN_CHECK_MODE);
    }

    public AlbumDetailSimpleAdapter(Context context, SyncStateShowScene scene) {
        super(context, scene);
    }

    public AlbumDetailSimpleAdapter(Context context, SyncStateShowScene type, int syncStateDisplayOptions) {
    ...
    public void doBindData(View view, Context context, Cursor cursor) {
        ...
        if (this.mSortBy == SortBy.SIZE) {
            ...
            long supportedSpecialTypeFlags;
            long specialTypeFlags = cursor.getLong(20);
            item.bindFileSize(0);
            if (isSecretAlbum()) {
                supportedSpecialTypeFlags = SecretAlbumConfig.getSupportedSpecialTypeFlags(specialTypeFlags);
            } else {
                supportedSpecialTypeFlags = specialTypeFlags;
            }
            item.bindIndicator(type, duration, supportedSpecialTypeFlags);
        }
        if (!isSecretAlbum()) {
            if (isFavorite(cursor.getPosition())) {
                ...
                item.setContentDescription(TalkBackUtil.getContentDescriptionForImage(context, cursor.getLong(5), cursor.getString(6), cursor.getString(10)));
