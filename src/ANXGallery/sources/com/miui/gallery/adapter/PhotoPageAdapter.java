...
public class PhotoPageAdapter extends PagerAdapter implements OnImageLoadFinishListener, TransitionListener {
    ...
    public int getItemPosition(Object object) {
        if (this.isPreviewing) {
            ...
            if (isItemValidate(object) && isTypeMatch(object)) {
                return -3;
            }
            return -2;
        }
    ...
    private boolean isTypeMatch(Object object) {
        if (!isItemValidate(object)) {
            return false;
        }
        PhotoPageItem item = (PhotoPageItem) object;
        Integer type = (Integer) item.getTag(R.id.tag_view_type);
        BaseDataItem curDataItem = getDataItem(((Integer) item.getTag(R.id.tag_item_position)).intValue());
        if (curDataItem == null) {
            return false;
        }
