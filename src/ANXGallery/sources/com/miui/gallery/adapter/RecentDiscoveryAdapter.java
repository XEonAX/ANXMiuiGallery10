...
public class RecentDiscoveryAdapter extends AlbumDetailSimpleAdapter implements StickyGridHeadersBaseAdapter {
    ...
    public RecentDiscoveryAdapter(Context context) {
        this(context, SyncStateShowScene.SCENE_IN_CHECK_MODE, false);
    }

    public RecentDiscoveryAdapter(Context context, SyncStateShowScene scene, boolean isInPickMode) {
        super(context, scene);
    ...
    private void recordAlbumClick(String albumName) {
        ...
        GallerySamplingStatHelper.recordCountEvent("recent_album", "recent_album_click_album_name", params);
    }
    ...
    private Intent newAlbumFromClickedIntent(long albumId) {
        ...
        int unwriteable = albumLocalPath.startsWith("/") ? 1 : 0;
    ...
    public View getDividerView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == this.mGroupStartPos.size() - 1) {
            ...
            convertView = LayoutInflater.from(this.mContext).inflate(R.layout.recent_section_divider_transparent, parent, false);
            convertView.setTag("last_divider");
        } else if (TextUtils.equals(tag, "normal_divider")) {
            return convertView;
        } else {
            convertView = LayoutInflater.from(this.mContext).inflate(R.layout.recent_section_divider, parent, false);
    ...
    public Cursor swapCursor(Cursor newCursor) {
        if (newCursor != null && newCursor.getExtras().containsKey("extra_timeline_item_count_in_group") && newCursor.getExtras().containsKey("extra_timeline_group_start_pos") && newCursor.getExtras().containsKey("extra_timeline_group_time_labels")) {
            ...
            this.mGroupStartTimeLabels = newCursor.getExtras().getStringArrayList("extra_timeline_group_time_labels");
        } else {
            if (this.mGroupItemCount != null) {
