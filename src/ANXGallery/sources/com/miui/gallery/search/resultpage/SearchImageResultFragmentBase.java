...
public abstract class SearchImageResultFragmentBase extends SearchResultFragmentBase implements RequestLoadMoreListener {
    ...
    protected void goToPhotoPage(AdapterView<?> adapterView, int position, String from) {
        ImageLoadParams item = new ImageLoadParams(this.mAdapter.getItemKey(position), this.mAdapter.getLocalPath(position), getMicroThumbnailSize(), this.mAdapter.getItemDecodeRectF(), position, this.mAdapter.getMimeType(position), this.mAdapter.getFileLength(position));
