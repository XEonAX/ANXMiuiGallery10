...
public class Config {
    ...
    public static class ImageLoaderConfig {
        public static ImageLoaderConfiguration getConfig(Context context) {
            ...
            ImageLoaderConfiguration.Builder builder = new ImageLoaderConfiguration.Builder(context).thumbnailCache(diskCache).diskCacheSize(31457280).memoryCache(new LRULimitedMemoryCache(ThumbConfig.get().sImageMemoryCache)).threadPoolSize(4).threadPriority(4).tasksProcessingOrder(QueueProcessingType.FIFO).imageDecoder(new ImageDecoderSupportRegion(isLoggingEnabled));
            if (isLoggingEnabled) {
    ...
    public static class ThumbConfig {
        ...
        private ThumbConfig() {
            ...
            microThumbSize = Math.min(screenWidth, screenHeight) / resources.getInteger(R.integer.album_page_header_columns);
            this.sAlbumHeaderThumbTargetSize = new ImageSize(microThumbSize, (int) (((double) microThumbSize) / 1.5d));
            ...
            this.sImageMemoryCache = Math.max((int) (((1.6f * ((float) screenWidth)) * ((float) screenHeight)) * ((float) onePixelSize)), nexEngine.ExportHEVCHighTierLevel61);
            this.sPreloadNum = Math.max(((int) ((1.0f * ((float) this.sImageMemoryCache)) / ((float) ((microThumbSize * microThumbSize) * onePixelSize)))) - ((int) Math.ceil((double) (((1.0f * ((float) screenWidth)) * ((float) screenHeight)) / ((float) (microThumbSize * microThumbSize))))), 0);
        }
