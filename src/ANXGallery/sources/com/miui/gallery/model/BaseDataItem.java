...
public class BaseDataItem implements Serializable {
    ...
    public BaseDataItem setLatitude(float latitude) {
        this.mLatitude = latitude;
        return this;
    }

    public BaseDataItem setLongitude(float longitude) {
    ...
    public boolean isVideo() {
        return TextUtils.isEmpty(this.mMimeType) ? false : FileMimeUtil.isVideoFromMimeType(this.mMimeType);
    }

    public boolean isGif() {
        return TextUtils.isEmpty(this.mMimeType) ? false : FileMimeUtil.isGifFromMimeType(this.mMimeType);
    }
    ...
    public NSGifDecode createNSGifDecoder(JobContext jc) {
        if (!TextUtils.isEmpty(getOriginalPath())) {
            ...
            try {
                ...
                MiscUtil.closeSilently(fileDescriptor);
            }
        }
        return null;
    }
    ...
    public void getCoordidate(float[] latLong) {
        if (latLong != null) {
    ...
    protected void onCacheLoaded() {
        long size = FileUtils.getFileSize(this.mFilePath);
        if (size <= 0) {
            return;
        }
        if (size != this.mSize || this.mWidth == 0 || this.mHeight == 0) {
            this.mSize = size;
            try {
