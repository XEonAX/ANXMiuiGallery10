...
public class LocationManager {
    ...
    public void appendDefaultLocationValues(ContentValues values) {
        ...
        if (!TextUtils.isEmpty(latitude) && !TextUtils.isEmpty(longitude)) {
            hasLatLng = true;
            lat = LocationUtil.convertRationalLatLonToFloat(latitude, latitudeRef);
            lng = LocationUtil.convertRationalLatLonToFloat(longitude, longitudeRef);
    ...
    public void recordMediaLocation(String path, Intent intent) {
        ...
        if (intent.hasExtra("extra_location_option")) {
            ...
            GallerySamplingStatHelper.recordCountEvent("location_manager", "location_manager_gps_status", params);
        ...
        if (cacheLocation != null) {
            ...
            GallerySamplingStatHelper.recordCountEvent("location_manager", "location_manager_gps_delay_min", params);
            if (delay < 0 || delay > 60) {
    ...
    public static String formatExifGpsString(String lat, String lng, String latRef, String lngRef) {
        ...
        return String.format("%s,%s", new Object[]{Float.valueOf(LocationUtil.convertRationalLatLonToFloat(lat, latRef)), Float.valueOf(LocationUtil.convertRationalLatLonToFloat(lng, lngRef))});
    }
    ...
    private static void recordStatistics(Statistics stat) {
        ...
        GallerySamplingStatHelper.recordCountEvent("location_manager", "location_manager_load_stat", params);
    }
    ...
    private boolean loadLocationForItem(Context context, CloudItem item, boolean foreground, Statistics stat) throws Exception {
        ...
        if (isLatLngValid((double) item.lat, (double) item.lng)) {
    ...
    private Address getAddress(Context context, Locale locale, CloudItem item, boolean foreground, Statistics stat) throws Exception {
        ...
        CacheItem cacheItem = this.mAddressCache.get(locale, (double) item.lat, (double) item.lng);
        if (cacheItem != null) {
        ...
        List<Address> addresses = geocoder.getFromLocation((double) item.lat, (double) item.lng, 1);
        long cost = System.currentTimeMillis() - start;
        stat.cost = (int) (((long) stat.cost) + cost);
        if (foreground) {
            Log.d("LocationManager", "get location cost: %s", Long.valueOf(cost));
        }
        if (!MiscUtil.isValid(addresses)) {
            return address;
        }
        address = fixAddressFault((Address) addresses.get(0));
        this.mAddressCache.put(locale, (double) item.lat, (double) item.lng, address);
        return address;
    }
    ...
    private CloudItem parseCursor(Cursor cursor) {
        ...
        if (TextUtils.isEmpty(lat) || TextUtils.isEmpty(lng)) {
            ...
            item.lat = LocationUtil.convertRationalLatLonToFloat(lat, latRef);
            item.lng = LocationUtil.convertRationalLatLonToFloat(lng, lngRef);
        }
        return item;
    }
