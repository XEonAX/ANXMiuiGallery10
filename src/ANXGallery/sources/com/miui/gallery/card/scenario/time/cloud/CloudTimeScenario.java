...
public class CloudTimeScenario extends TimeScenario {
    ...
    public boolean onPrepare(List<Record> records, List<Card> cards) {
        if (MiscUtil.isValid(records) || MiscUtil.isValid(cards) || this.mStartTime <= 0 || this.mEndTime <= this.mStartTime || TextUtils.isEmpty(this.mTitle) || TextUtils.isEmpty(this.mDescription)) {
            return false;
        }
