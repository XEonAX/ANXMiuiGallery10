...
public abstract class TimeScenario extends Scenario {
    protected static final String NON_CAMERA_SELECTION = (Constants.NON_CAMERA_BASE_SELECTION + " AND " + "_id" + " IN (%s)");
    protected static final String TIME_SELECTION = (Constants.CAMERA_BASE_SELECTION + " AND " + "mixedDateTime" + " > %s AND " + "mixedDateTime" + " < %s");
    ...
    protected List<ImageFeature> getCameraMediaIdsByStartEndTimeTags(Integer[] tags, long startTime, long endTime) {
        ...
        if (MiscUtil.isValid(images)) {
            ...
            if (MiscUtil.isValid(nonCameraImages)) {
    ...
    protected List<Long> getRecordStartTimesFromRecordAndCards(List<Record> records, List<Card> cards) {
        List<Long> recordStarts = new ArrayList();
        if (MiscUtil.isValid(records)) {
