...
public class ScenarioAlbumTask extends ScenarioTask {
    ...
    private void statScenarioCreateFailed() {
        ...
        GallerySamplingStatHelper.recordCountEvent("assistant", "assistant_card_create_failed", params);
    }
    ...
    public CardResult generateCard(JSONObject data, Record record, boolean processImages) {
        ...
        if (MiscUtil.isValid(mediaFeatureItems)) {
            ...
            if (MiscUtil.isValid(unProcessedMediaFeatureItems) && !processImages) {
                return CardResult.HAVE_UNPROCESSED_IMAGES;
            }
            if (processImages(data, unProcessedMediaFeatureItems, false)) {
                ...
                Uri detailUrl = CardUtil.getAlbumUri();
                Builder imageUri = new Builder(GalleryApp.sGetAndroidContext()).setTitle(scenario.generateTitle(record, selectImages)).setDescription(scenario.generateDescription(record, selectImages)).setDeletable(scenario.isDeletable()).setImageUri(null);
                if (detailUrl == null) {
                    str = null;
                } else {
                ...
                if (MiscUtil.isValid(existCardList)) {
                ...
                GallerySamplingStatHelper.recordCountEvent("assistant", "assistant_card_created_success", params);
