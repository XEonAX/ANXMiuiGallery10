...
public abstract class BaseImageTask extends PendingTask<JSONObject> {
    ...
    private static class ImageGuaranteeJob implements Job<String> {
        ...
        ImageGuaranteeJob(MediaFeatureItem mediaFeatureItem, DownloadType downloadType) {
    ...
    protected boolean processImages(JSONObject data, List<MediaFeatureItem> mediaFeatureItems, boolean calculateClusterGroup) {
        if (!MiscUtil.isValid(mediaFeatureItems)) {
            return true;
        }
        CardUtil.bindImageFeatures(mediaFeatureItems);
        if (LibraryManager.getInstance().loadLibrary(LibraryConstants.sStoryLibraries)) {
            ...
            Future<String> future = getImageFilePath((MediaFeatureItem) mediaFeatureItems.get(0), downloadType);
            for (int i = 1; i < mediaFeatureItems.size(); i++) {
                String path = (String) future.get();
                future = getImageFilePath((MediaFeatureItem) mediaFeatureItems.get(i), downloadType);
        ...
        try {
            ...
            jsonObject.put("libraryIds", GsonUtils.toString(LibraryConstants.sStoryLibraries));
    ...
    protected static List<MediaFeatureItem> getSelectedImages(List<MediaFeatureItem> list) {
        ...
        if (MiscUtil.isValid(imageFeatureGroups)) {
    ...
    public static Future<String> getImageFilePath(MediaFeatureItem mediaFeatureItem, DownloadType downloadType) {
        return ThreadManager.getMiscPool().submit(new ImageGuaranteeJob(mediaFeatureItem, downloadType));
    }
    ...
    List<Long> getAllProcessedImages() {
        List<Long> result = new ArrayList();
        Cursor cursor = GalleryEntityManager.getInstance().rawQuery(ImageFeature.class, new String[]{"imageId"}, ImageFeature.ALL_PROCESSED_SELECTION + " OR " + "imageType" + "=" + -1, null, null, null, "imageId ASC", null);
        if (cursor != null) {
    ...
    protected static List<MediaFeatureItem> queryMediaItemByIds(List<Long> ids) {
        return queryMediaItem(Constants.CAMERA_BASE_SELECTION + " AND " + "_id" + " IN ('" + TextUtils.join("','", ids) + "')" + " AND " + "(localFlag NOT IN (11, 0, -1, 2) OR (localFlag=0 AND serverStatus='custom'))");
    }
