...
public class StatisticsJob extends AbstractJob {
    private void recordGallerySettings() {
        ...
        GallerySamplingStatHelper.recordStringPropertyEvent("settings", "has_xiaomi_account", String.valueOf(hasXiaomiAccount));
        if (hasXiaomiAccount) {
            ...
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_cloud_backup_on_c", String.valueOf(isCloudBackupOn));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_local_mode_on_c", String.valueOf(isLocalModeOn));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_face_album_on_c", String.valueOf(isFaceAlbumOn));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_backup_only_wifi_on_c", String.valueOf(isBackupOnlyWifi));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_ai_album_on_c", String.valueOf(isAIAlbumOn));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_search_ai_album_enabled_c", String.valueOf(isSearchAIAlbumEnabled));
            GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_search_bar_enabled_c", String.valueOf(isSearchBarEnabled));
            if (isSearchAIAlbumEnabled) {
                GallerySamplingStatHelper.recordStringPropertyEvent("settings", "is_search_user_switch_on_c", String.valueOf(isSearchUserSwitchOn));
            }
    ...
    private void recordSyncState() {
        ...
        Account account = AccountHelper.getXiaomiAccount(GalleryApp.sGetAndroidContext());
        GallerySamplingStatHelper.recordStringPropertyEvent("sync_state_analysis", "login_account", String.valueOf(account != null));
        if (account != null) {
            ...
            GallerySamplingStatHelper.recordStringPropertyEvent("sync_state_analysis", "master_sync", String.valueOf(masterSync));
            if (masterSync) {
                GallerySamplingStatHelper.recordStringPropertyEvent("sync_state_analysis", "gallery_sync", String.valueOf(gallerySync));
            }
            if (!gallerySync) {
                isEverSynced = isEverSynced();
                GallerySamplingStatHelper.recordStringPropertyEvent("sync_state_analysis", "sync_off_ever_synced", String.valueOf(isEverSynced));
                CloudSpaceInfo info = SyncStateUtil.getCloudSpaceInfo(GalleryApp.sGetAndroidContext());
                total = info.getTotal();
                if (total > 0) {
                    GallerySamplingStatHelper.recordNumericPropertyEvent("sync_state_analysis", "sync_off_cloud_space_state", (long) ((int) ((100.0f * ((float) (total - info.getUsed()))) / ((float) total))));
                }
            }
        } else {
            isEverSynced = false;
        ...
        if (!isEverSynced) {
            ...
            if (total > 0) {
                GallerySamplingStatHelper.recordNumericPropertyEvent("sync_state_analysis", "sync_off_local_space_state", (long) ((int) ((100.0f * ((float) StorageUtils.getAvailableBytes(primaryVolume))) / ((float) total))));
            }
        }
    }
    ...
    private void recordOtherProperties() {
        ...
        if (hasExternalSDCard) {
            ...
            GallerySamplingStatHelper.recordStringPropertyEvent("storage", "storage_can_write_external_sd_card", String.valueOf(StorageUtils.canWrite(context, StorageUtils.getSecondaryStoragePath())));
        }
    }
    ...
    private int querySpecialMediaCountOfSortByModifyAlbum(SQLiteDatabase db) {
        ...
        if (MiscUtil.isValid(albumsOrderByDateModified)) {
    ...
    private void recordAlbumsAndImagesCount() {
        ...
        if (mediaCountOfAllAlbums != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "image_count_of_all_albums", (long) mediaCountOfAllAlbums);
        }
        if (videoCountOfAllAlbums != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "video_count_of_all_albums", (long) videoCountOfAllAlbums);
        }
        if (mediaCountOfCameraAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "image_count_of_camera_album", (long) mediaCountOfCameraAlbum);
        }
        if (videoCountOfCameraAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "video_count_of_camera_album", (long) videoCountOfCameraAlbum);
        }
        if (mediaCountOfScreenshotAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "image_count_of_screenshot_album", (long) mediaCountOfScreenshotAlbum);
        }
        if (mediaCountOfDownloadAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "image_count_of_download_album", (long) mediaCountOfDownloadAlbum);
        }
        if (albumCountOfCloudBackup != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "album_count_of_cloud_backup", (long) albumCountOfCloudBackup);
        }
        if (albumCountOfOwnerCreate != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "album_count_of_owner_create", (long) albumCountOfOwnerCreate);
        }
        if (albumCountOfOtherShare != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "album_count_of_other_share", (long) albumCountOfOtherShare);
        }
        if (mediaCountOfFavoritesAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "image_count_of_favorites_album", (long) mediaCountOfFavoritesAlbum);
        }
        if (specialMediaCountOfSortByModifyAlbum != -1) {
            GallerySamplingStatHelper.recordNumericPropertyEvent("albums_and_images_count", "special_image_count_of_sort_by_modify_and_show_in_home_album", (long) specialMediaCountOfSortByModifyAlbum);
        }
    }
    ...
    private void recordMediasSpace() {
        ...
        if (totalSpace != -1) {
            GallerySamplingStatHelper.recordStringPropertyEvent("medias_space", "medias_space_of_all", formatSize(totalSpace));
            ...
            if (photoSpace != -1) {
                GallerySamplingStatHelper.recordNumericPropertyEvent("medias_space", String.format(Locale.US, "medias_space_ratio_of_album_%s", new Object[]{"Camera-Image"}), formatProportion((1.0f * ((float) photoSpace)) / ((float) totalSpace)));
            }
            photoSpace = queryMediaSpace(db, "serverType=2 AND " + String.format(Locale.US, albumFormat, new Object[]{"DCIM/Camera"}));
            if (photoSpace != -1) {
                GallerySamplingStatHelper.recordNumericPropertyEvent("medias_space", String.format(Locale.US, "medias_space_ratio_of_album_%s", new Object[]{"Camera-Video"}), formatProportion((1.0f * ((float) photoSpace)) / ((float) totalSpace)));
            }
            long videoSpace = queryMediaSpace(db, "serverType=2");
            if (videoSpace != -1) {
                GallerySamplingStatHelper.recordNumericPropertyEvent("medias_space", String.format(Locale.US, "medias_space_ratio_of_album_%s", new Object[]{"Video"}), formatProportion((1.0f * ((float) videoSpace)) / ((float) totalSpace)));
            }
            ...
            if (attributes != -1 && (((long) attributes) & 64) == 0) {
                ...
                if (weChatSpace != -1) {
                    GallerySamplingStatHelper.recordNumericPropertyEvent("medias_space", String.format(Locale.US, "medias_space_ratio_of_album_%s", new Object[]{"weixin"}), formatProportion((1.0f * ((float) weChatSpace)) / ((float) totalSpace)));
                }
            }
        }
    }
    ...
    private void recordPeopleProperties() {
        ...
        GallerySamplingStatHelper.recordNumericPropertyEvent(str, str2, j);
    }
