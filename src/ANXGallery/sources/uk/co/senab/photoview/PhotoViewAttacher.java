...
public class PhotoViewAttacher implements OnTouchListener, OnGlobalLayoutListener, IPhotoView, OnGestureListener {
        ...
        class AdjustAnimation implements Runnable {
            ...
            private void updateMatrix() {
                if (PhotoViewAttacher.this.getImageView() != null) {
                    ...
                    PhotoViewAttacher.this.dispatchScaleChanged(Math.min(wScaleFactor, hScaleFactor), baseDisplay.centerX(), baseDisplay.centerY(), PhotoViewAttacher.this.getScale());
                }
            }
            ...
            private void checkBounds() {
                ...
                if (imageView != null) {
                    ...
                    if (!MiscUtil.floatEquals(degrees, (float) finalDegrees)) {
                    ...
                    if (!MiscUtil.floatEquals(wscale, 1.0f) || !MiscUtil.floatEquals(hscale, 1.0f)) {
    ...
    private class Transition implements Runnable {
        ...
        public void ensureAlpha(boolean enter) {
            float alpha = enter ? MiscUtil.floatEquals(PhotoViewAttacher.this.mAlpha, 1.0f) ? 0.0f : PhotoViewAttacher.this.mAlpha : MiscUtil.floatEquals(PhotoViewAttacher.this.mAlpha, 0.0f) ? 1.0f : PhotoViewAttacher.this.mAlpha;
    ...
    private boolean interceptDrawTiles() {
        return !this.mIsRegionDecodeEnable || getTransition().isRunning() || false || (this.mRotateDetector != null && this.mRotateDetector.isRotating());
    }
    ...
    public void onScale(float scaleFactor, float focusX, float focusY) {
        if (this.mZoomEnabled) {
            ...
            if (preScale < getMaxPointsScalingScale() || scaleFactor < 1.0f) {
                ...
                checkAndDisplayMatrix();
                float curScale = getScale();
                dispatchScaleChanged(scaleFactor, focusX, focusY, curScale);
    ...
    private void dispatchScaleChanged(float scaleFactor, float focusX, float focusY, float scale) {
        if (this.mScaleChangeListener != null) {
            this.mScaleChangeListener.onScaleChange(scaleFactor, focusX, focusY, scale);
        }
    }
    ...
    public void animEnter(ItemViewInfo enterInfo, TransitionListener listener) {
        if (enterInfo != null && getTransition() != null) {
            getTransition().stop();
            RectF rect = getDisplayRect();
            if (rect != null) {
                float scale;
                ...
                if (((float) enterInfo.getWidth()) < ratioWidth || ((float) enterInfo.getHeight()) < ratioHeight) {
                    ...
                    float clipX = MiscUtil.clamp(((float) enterInfo.getWidth()) / ratioWidth, 0.0f, 1.0f);
                    clipY = MiscUtil.clamp(clipY, 0.0f, 1.0f);
    ...
    public void animExit(ItemViewInfo exitInfo, TransitionListener listener) {
        if (exitInfo != null && exitInfo.isLocationValid() && getTransition() != null) {
            getTransition().stop();
            RectF rect = getDisplayRect(getDrawMatrix());
            if (rect != null) {
                float scale;
                ...
                if (((float) exitInfo.getWidth()) < ratioWidth || ((float) exitInfo.getHeight()) < ratioHeight) {
                    getTransition().clip(1.0f, 1.0f, (((float) exitInfo.getWidth()) / ratioWidth) - 1.0f, MiscUtil.clamp(((float) exitInfo.getHeight()) / ratioHeight, 0.0f, 1.0f) - 1.0f);
                }
    ...
    private boolean needDrawTile() {
        ...
        if (!(this.mTileView == null || imageView == null || imageView.getDrawable() == null)) {
            ...
            if (MiscUtil.floatNear((1.0f * ((float) drawableWidth)) / ((float) drawableHeight), (1.0f * ((float) tileWidth)) / ((float) tileHeight), 0.5f)) {
                ...
                if (MiscUtil.floatEquals((float) (Math.min(drawableWidth, drawableHeight) / Math.max(drawableWidth, drawableHeight)), (float) (Math.min(tileWidth, tileHeight) / Math.max(tileWidth, tileHeight)))) {
                    ...
                    GallerySamplingStatHelper.recordErrorEvent("photo", "photo_tile_orientation_error", params);
                }
            }
        }
        return false;
    ...
    private void updateAlpha(float alpha) {
        if (alpha < 1.0f) {
            this.mAlpha = MiscUtil.clamp(alpha, 0.0f, 1.0f);
        } else {
            this.mAlpha = 1.0f;
        }
