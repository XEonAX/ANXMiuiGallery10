...
public class CompatibleManager {
    ...
    public boolean isCompatible() {
        ...
        if (a(this.dl.getSdkDependency().getLevel(), PackageConstants.CURRENT_SDK_LEVEL)) {
            PackageManager packageManager = this.mContext.getPackageManager();
            if (packageManager == null) {
                Log.w(PackageConstants.LOG_TAG, "package manager is not ready yet");
                return true;
            }
            for (String str2 : this.dl.getDependencies().keySet()) {
                Dependency dependency = this.dl.getDependency(str2);
    ...
    public boolean isCurrent() {
        return this.dl.getSdkDependency().getLevel().getTargetLevel() == PackageConstants.CURRENT_SDK_LEVEL;
    }

    public int getLevel() {
        return this.dl.getModule().getLevel();
    }
}
