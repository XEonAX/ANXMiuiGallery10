...
public class ManifestParser {
    ...
    private static final String fH = "com.miui.sdk.manifest";
    private static final Map<String, String> xA = new HashMap(3);
    private static final String xv = "miui_manifest";
    private static final String xw = "manifest";
    private static final String xx = "uses-sdk";
    private static final String xy = "modules";
    private static final String xz = "module";
    private XmlResourceParser fU;
    private Resources nO;

    static {
        xA.put(PackageConstants.PACKAGE_NAME, "miui");
        xA.put("com.miui.system", "miui.system");
        xA.put("com.miui.rom", "android.miui");
    }

    private ManifestParser(Resources resources, XmlResourceParser xmlResourceParser) {
        this.nO = resources;
        this.fU = xmlResourceParser;
    }
    ...
    public static ManifestParser createFromResources(Resources resources, String str, Bundle bundle) {
        ...
        if (i == 0) {
            String str2 = (String) xA.get(str);
            if (str2 != null) {
                str = str2;
            }
            i = resources.getIdentifier(xv, "xml", str);
        }
        return createFromXmlParser(resources, i == 0 ? null : resources.getXml(i));
    }
    ...
    public Manifest parse(Map<String, Object> map) {
        ...
        if (this.fU != null) {
            try {
                ...
                XmlResourceParser xmlResourceParser = this.fU;
                while (true) {
                    int next = xmlResourceParser.next();
                    if (next == 2 || next == 1) {
                    }
                }
                if (xw.equals(xmlResourceParser.getName())) {
    ...
    private void c(Manifest manifest, Resources resources, XmlResourceParser xmlResourceParser) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next != 3) {
                if (next != 4) {
                    if (xz.equals(xmlResourceParser.getName())) {
