...
public class AlmanacConsPros {
    ...
    private void de() {
        String directIndexedFilePath = DirectIndexedFileExtractor.getDirectIndexedFilePath(AppConstants.getCurrentApplication(), Fi);
        if (directIndexedFilePath != null && new File(directIndexedFilePath).exists()) {
            try {
                this.FX = DirectIndexedFile.open(directIndexedFilePath);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        if (this.FX == null) {
            try {
                this.FX = DirectIndexedFile.open(AppConstants.getCurrentApplication().getResources().getAssets().open(Fi, 1));
    ...
    public String[] getConsPros(long j) {
        if (this.FX == null || j < GP.longValue() || j > GS.longValue()) {
            return null;
        }
        String[] strArr = new String[2];
        String[] split = ((String) this.FX.get(0, (int) ((j - GP.longValue()) / 86400000), 0)).split(",", -1);
            ...
            for (int i = 0; i < split.length; i++) {
                ...
                if (!TextUtils.isEmpty(split[i])) {
                    for (String parseInt : split[i].split("、")) {
                        ...
                        if (parseInt2 < GV.length) {
                            arrayList.add(GV[parseInt2]);
                        }
                    }
                }
