...
public class BasicSeekBar extends SeekBar {
    ...
    public void setThumb(Drawable thumb) {
        if (thumb instanceof LayerDrawable) {
            ...
            if (!(layer.findDrawableByLayerId(R.id.refocus_seekbar_thumb_active_state) == null || layer.findDrawableByLayerId(R.id.refocus_seekbar_thumb_reset_state) == null)) {
                this.mThumb = layer;
                updateThumb(getProgress());
                return;
            }
    ...
    void updateThumb(int progress) {
        if (this.mThumb != null) {
            if (progress == 0) {
                super.setThumb(this.mThumb.findDrawableByLayerId(R.id.refocus_seekbar_thumb_reset_state));
            } else {
                super.setThumb(this.mThumb.findDrawableByLayerId(R.id.refocus_seekbar_thumb_active_state));
            }
        }
    }
}
